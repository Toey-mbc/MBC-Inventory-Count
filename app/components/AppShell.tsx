...
export default function AppShell({children}:{children:React.ReactNode}){
 ...
 return <div className="shell"><aside className="sidebar"><div className="brand">MBC Inventory</div><nav className="nav">{links.map(([href,label])=><Link key={href} href={href} style={{background:pathname===href?'#1b3b2e':undefined}}>{label}</Link>)}</nav><button className="btn secondary" style={{marginTop:20,width:'100%'}} onClick={logout}>ออกจากระบบ</button></aside><main className="main">{children}</main></div>
}
