...
export default function Dashboard(){
 ...
 return <Protected>
  ...
  <div className="grid two-col">
   ...
   <div className="card"><div className="card-title"><h2>สรุปตามสภาพสินค้า</h2><ClipboardList size={18} className="muted"/></div><div className="prototype-condition-list">
    {Object.entries(conditions).length?Object.entries(conditions).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([key,value])=><div className="condition-pill" key={key}><span>{conditionLabel(key)}</span><strong>{value.toLocaleString()}</strong></div>):<div className="empty">ยังไม่มีข้อมูลการนับ</div>}
   </div><div className="unknown-panel"><span>บาร์โค้ดไม่พบ</span><strong>{kpi.unknown.toLocaleString()}</strong><a href="/variance" className="btn secondary small">ตรวจสอบรายการ</a></div><div className="notice" style={{marginTop:12,display:'flex',gap:8,alignItems:'center'}}><HelpCircle size={16}/><span>ยอดที่นับรวม {kpi.quantity.toLocaleString()} ชิ้น · ไม่รวมบาร์โค้ดที่ยังไม่ผูกสินค้า</span></div></div>
  </div>
