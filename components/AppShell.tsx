...
import {BarChart3,Bell,Boxes,Building2,ChevronRight,ClipboardCheck,FileBarChart2,LogOut,Menu,PackageSearch,ScanLine,Settings,ShieldCheck,Users,WifiOff,X} from 'lucide-react'
...
const nav:NavItemType[]=[
 {href:'/dashboard',label:'Dashboard',icon:BarChart3,group:'main',roles:['admin','warehouse_manager','sale_support','counter','viewer']},
 ...
 {href:'/users',label:'ผู้ใช้งานและสิทธิ์',icon:Users,group:'system',roles:['admin']},
...
export default function AppShell({children,profile}:{children:React.ReactNode;profile:Profile}){
 ...
 const[open,setOpen]=useState(false);const[online,setOnline]=useState(true);const[org,setOrg]=useState({name:'MBC Communications',system_name:'Inventory Count System'});const[activeRound,setActiveRound]=useState<{code:string;name:string}|null>(null)
 useEffect(()=>setOpen(false),[pathname])
 useEffect(()=>{supabase.from('app_settings').select('value').eq('key','organization').maybeSingle().then(({data})=>{if(data?.value)setOrg(v=>({...v,...data.value}))});supabase.from('count_rounds').select('code,name').eq('status','active').order('created_at',{ascending:false}).limit(1).maybeSingle().then(({data})=>setActiveRound(data||null))},[supabase])
 ...
 return <div className="app-shell">
  ...
  <aside className={`sidebar ${open?'open':''}`}>
   <div className="brand-block"><div className="brand-logo"><span>M</span></div><div><div className="brand-name">{org.name}</div><div className="brand-sub">{org.system_name}</div></div><button className="sidebar-close" onClick={()=>setOpen(false)}><X size={20}/></button></div>
